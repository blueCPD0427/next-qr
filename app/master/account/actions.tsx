...
const MasterAccountSchema = z.object({
    id: z.string().min(1, { message: 'システムエラーが発生しました。' }),
    name: z.string().min(1, { message: '名前の入力は必須です。' }),
    postCode: z
        .string()
...
const CreateMasterAccountRefined = CreateMasterAccount.refine(
    (data) =>
        data.password != '' &&
...
export async function updateMasterAccountApi(formData: MasterAccountForm) {
    ...
    if (formData.password != '') {
        UpdateMasterAccountRefined = MasterAccountSchema.refine(
            (data) =>
                data.password != '' &&
                data.confirmPassword != '' &&
                data.password === data.confirmPassword,
        ...
        const OmitPasswordSchema = MasterAccountSchema.omit({
            password: true,
            confirmPassword: true,
        });

        UpdateMasterAccountRefined = OmitPasswordSchema.superRefine(
            async (data, ctx) => {
                const duplicateEmail = await prisma.masters.findFirst({
                    where: {
                        email: data.email,
