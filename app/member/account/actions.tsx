...
const MemberAccountSchema = z.object({
    id: z.string(),
    lastName: z.string().min(1, { message: '名前（姓）の入力は必須です。' }),
    firstName: z.string().min(1, { message: '名前（名）の入力は必須です。' }),
    ...
    address: z.string().min(1, { message: '住所の入力は必須です。' }),
    email: z
        .string()
...
const CreateMemberAccountRefined = CreateMemberAccount.refine(
    (data) =>
        data.password != '' &&
    ...
    try {
        const validatedFields = await CreateMemberAccountRefined.parseAsync({
            ...
            birthday:
                formData.birthdayY +
                '-' +
                lPadNum(formData.birthdayM, 2) +
                '-' +
                lPadNum(formData.birthdayD, 2),
        });
...
export async function updateMemberAccountApi(formData: MemberAccountForm) {
    ...
    if (formData.password != '') {
        UpdateMemberAccountRefined = MemberAccountSchema.refine(
            (data) =>
                data.password != '' &&
                data.confirmPassword != '' &&
                data.password === data.confirmPassword,
        ...
        const OmitPasswordSchema = MemberAccountSchema.omit({
            password: true,
            confirmPassword: true,
        });

        UpdateMemberAccountRefined = OmitPasswordSchema.superRefine(
            async (data, ctx) => {
                const duplicateEmail = await prisma.members.findFirst({
                    where: {
                        email: data.email,
    ...
    try {
        ...
        const { lastName, firstName, sex, postCode, address, email, birthday } =
            validatedFields;
